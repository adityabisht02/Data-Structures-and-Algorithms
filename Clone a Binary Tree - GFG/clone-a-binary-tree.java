...
class Solution{
    public void addNodesToMap(Tree node,Map<Tree,Tree> map){
        if(node==null){
            return ;
        }
        Tree newnode=new Tree(node.data);
        map.put(node,newnode);
        addNodesToMap(node.left,map);
        addNodesToMap(node.right,map);
    }
    public void addConnections(Tree node,Map<Tree,Tree> map){
        if(node==null){
            return ;
        }
        ...
        addConnections(node.left,map);
        addConnections(node.right,map);
    }
    public Tree cloneTree(Tree tree){
      //do dfs add all nodes to hashmap
      //do dfs again and make connections
      Map<Tree,Tree> map=new HashMap<>();
      map.put(null,null);
      addNodesToMap(tree,map);
      addConnections(tree,map);
